...
const BlocVoitures = () => {
  ...
  const loadData = async () => {
    const response = await axios.get(
      "https://garage-studi-backend.up.railway.app/voitures"
    );
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      document.querySelector('input[name="kilometrage"]').value =
        maxKilometrage;
  ...
  const filterVoiture = data.filter(
    (voiture) =>
      voiture.km >= minKilometrage &&
      voiture.km <= kilometrage &&
      voiture.price >= minPrice &&
      voiture.price <= price &&
      voiture.yearsCirculation >= minYear &&
      voiture.yearsCirculation <= year
  );

  const augmenterVue = () => {
    try {
      axios.put(
        `https://garage-studi-backend.up.railway.app/augmenter-vues-voitures`,
        { marqueVoiture }
      );
    } catch (error) {
      console.log(error);
      ...
      <div className="conteneurVoitures">
        ...
        <div className="filtreVoiture">
          <div className="filtreKilometrage">
            ...
            />
            <span>
              {minKilometrage} - {kilometrage} km{" "}
            </span>
            <button
              className="boutonFiltre"
              onClick={() => {
